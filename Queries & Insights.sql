...
SELECT
    ...
    ( SUM(Total_Serious_Injuries) + SUM(Total_Minor_Injuries)) AS `Total Injured`,

    -- Percentages
    ROUND((( SUM(Total_Serious_Injuries) + SUM(Total_Minor_Injuries)) * 100.0 /
...
SELECT
    COUNT(CASE WHEN f.Part_Name LIKE '%ING_ENG%' THEN 1 END) AS `Total Ingestion`,
    ROUND(
        (COUNT(CASE WHEN f.Part_Name LIKE '%ING_ENG%' THEN 1 END) * 100.0 /
         NULLIF(COUNT(CASE WHEN f.Part_Name LIKE '%STR%' THEN 1 END), 0)),
        2
...
SELECT 
    ...
    COUNT(DISTINCT f.INDEX_NR) AS `Total Bird Strikes`,
    ROUND(
        COUNT(DISTINCT f.INDEX_NR) * 100.0 / (SELECT COUNT(*) FROM faa),
        2
    ) AS `Percentage %`,
    ROUND(
        COUNT(CASE 
